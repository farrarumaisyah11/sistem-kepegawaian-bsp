...
/* Route print adalah fallback. Isi dibangun oleh engine A4 yang sama dengan tombol di Show. */
.jd-print-fallback-source{ display:none !important; }
</style>
@endsection
...
@php
    ...
    if (!empty($j->approved_at)) {
        try {
            $approvalDate = \Illuminate\Support\Carbon::parse($j->approved_at)->locale('id')->translatedFormat('d F Y H:i');
        } catch (\Throwable $e) {
...
(function () {
    ...
    async function savePdf(pages, filename) {
        ...
        // One DOM page is converted into exactly one PDF page. This prevents the blank-page issue
        // caused by handing one extremely tall export root to html2pdf.
        const firstPage = pages[0];
        const oldBreak = firstPage.style.breakAfter;
        const oldPageBreak = firstPage.style.pageBreakAfter;
        firstPage.style.breakAfter = 'auto';
        firstPage.style.pageBreakAfter = 'auto';

        let pdf;
        try {
            const worker = window.html2pdf().set(captureOptions(firstPage)).from(firstPage).toPdf();
            pdf = await worker.get('pdf');
        } finally {
            firstPage.style.breakAfter = oldBreak;
            firstPage.style.pageBreakAfter = oldPageBreak;
        }

        for (let index = 1; index < pages.length; index += 1) {
            const canvas = await renderPageCanvas(pages[index]);
            pdf.addPage('a4', 'portrait');
            pdf.addImage(canvas.toDataURL('image/jpeg', 1), 'JPEG', 0, 0, 210, 297, undefined, 'FAST');
        }
        pdf.save(filename);
    }
    ...
    async function downloadPdf(button) {
        ...
        try {
            ...
            console.error(error);
            alert('PDF tidak dapat dibuat. Pastikan file public/vendor/html2pdf/html2pdf.bundle.min.js tersedia, kemudian refresh halaman.');
        } finally {
            clearRoot();
            if (button) {
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Fallback route langsung memanggil dialog print native, tanpa halaman preview tambahan.
        setTimeout(function () {
            if (typeof window.printJabatanA4 === 'function') window.printJabatanA4();
        }, 180);
    });
</script>
@endif
